...
# CI Analyzer Setup Script
# Copies and customizes the ci-analyzer template for a target project

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" &> /dev/null && pwd)"
TEMPLATE_DIR="$SCRIPT_DIR/../.claude/agents/templates"
TEMPLATE_FILE="$TEMPLATE_DIR/ci-analyzer-template.md"
...
print_usage() {
    ...
    echo "  2. Copy and customize ci-analyzer template"
...
prompt_project_config() {
    echo -e "${BLUE}Configure CI Analyzer for your project:${NC}"
    echo ""
...
main() {
    ...
    echo -e "${GREEN}Setting up CI Analyzer for: $target_path${NC}"
    echo ""
    
    # Check if already exists
    local target_agents_dir="$target_path/.claude/agents"
    local target_file="$target_agents_dir/ci-analyzer.md"
    
    if [[ -f "$target_file" ]]; then
        echo -e "${YELLOW}Warning: ci-analyzer.md already exists${NC}"
    ...
    echo -e "${GREEN}✅ CI Analyzer successfully set up!${NC}"
    echo ""
    echo "Created: $target_file"
    echo ""
    echo -e "${BLUE}Next steps:${NC}"
    echo "1. Review and adjust $target_file if needed"
    echo "2. Test with: Use the ci-analyzer to investigate this build failure: [URL]"
    ...
    echo "- **CI/Build Analysis**: Use \`ci-analyzer\` subagent for GitHub Actions failures"
}
