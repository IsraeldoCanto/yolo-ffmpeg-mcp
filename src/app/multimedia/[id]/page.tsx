...
import { getMultimediaItem } from '@/lib/firebase/firestore';
...
export default function MultimediaDetailPage() {
  ...
  const [item, setItem] = useState<Multimedia | null>(null);
  ...
  const fetchItemData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const fetchedItem = await getMultimediaItem(itemId, user.uid);
      if (fetchedItem) {
        setItem(fetchedItem);
      } else {
        notFound(); 
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            ...
            <Link href={`/multimedia/${item.id}/edit`} passHref>
